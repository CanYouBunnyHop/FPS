...
void CalculateMainLight_float(float3 WorldPos, out float3 Direction, out float3 Color, out half DistanceAtten, out half ShadowAtten) //distanceAtten is diffues, shadowatten is specular
{
    ...
    float MainDiffuse, float MainSpecular, float3 MainColor, out float Diffuse, out float Specular, out float3 Color) 
{
    ...
    //If no light touches this pixel, set color to the main light color
    Color = total <=0 ? MainColor : Color / total;
#endif
}


void CalculateDiffuse_float(float3 Normals, float3 BaseColor,  out float3 FinalDiffuse)
{
#ifdef SHADERGRAPH_PREVIEW
    FinalDiffuse = BaseColor;
#else
    Light mainL = GetMainLight();
    float NL = normalize(dot(Normals, mainL.direction));

    FinalDiffuse = mainL.color * NL * BaseColor;
#endif
}
